...
# Creating the Model
# 
logreg <- glm(data=survey_data, vote_trump~as.factor(gender)+as.factor(race_ethnicity)+as.factor(household_income)+as.factor(education)+as.factor(age) + as.factor(hispanic), family = "binomial")

# logreg <- model <- glm(vote_trump ~ age + gender + race_asian_pacific + race_white + race_black + race_other, 
