  ...
  mutate(household_income = 
           case_when(
             (household_income == "$Less than $14,999" || household_income == "$15,000 to $19,999" || 
                household_income == "$20,000 to 24,999") ~ "Less than 24,999",
  ...
  mutate(education = 
           case_when(
             (education == "High school graduate" || education == " Other post high school vocational training" || 
                education == "3rd Grade or less" || education == "Completed some college, but no degree" ||
                education == "Completed some high school" || education == "Middle School - Grades 4 - 8") ~ "Highschool or less",
             ...
             (education == "Masters degree" || education == "Doctorate degree") ~ "Bachelors degree"
           )
  ) %>%
...
# reduced_data <- mutate(race_white = ifelse(race_ethnicity == "White" ,1,0))
# reduced_data <- reduced_data %>% mutate(race_black = ifelse(race_ethnicity == "Black, or African American" ,1,0))
# reduced_data <- reduced_data %>% mutate(race_other = ifelse(race_ethnicity == "	American Indian or Alaska Native"| race_ethnicity=="Some other race" ,1,0))
# 
# 
# reduced_data <- reduced_data %>% mutate(race_asian = ifelse(race_ethnicity== "Asian (Asian Indian)"|race_ethnicity == "Asian (Chinese)"|
#                                                             race_ethnicity == "Asian (Filipino)"|race_ethnicity == "Asian (Japanese)"|
#                                                             race_ethnicity == "Asian (Korean)" |race_ethnicity == "Asian (Korean)"|
#                                                             race_ethnicity == "Asian (Vietnamese)" |race_ethnicity == "Asian (Other)"|
#                                                             race_ethnicity == "Pacific Islander (Native Hawaiian)"|race_ethnicity == "Pacific Islander (Guamanian)"|
#                                                             race_ethnicity == "Pacific Islander (Samoan)" | race_ethnicity == "Pacific Islander (Other)" ,1,0))
# 
